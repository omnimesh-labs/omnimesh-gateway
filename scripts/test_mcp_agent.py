...
class MCPGatewayClient:
    ...
    def __init__(self, base_url: str = GATEWAY_BASE_URL):
        ...
        self.session = httpx.AsyncClient(timeout=30.0)  # Increased timeout for transport tests
    ...
    async def test_all_transports(self) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._test_streamable_http_transport(transport_results["streamable_http"])
    ...
    async def _test_json_rpc_transport(self, result: Dict[str, Any]):
        ...
        response.raise_for_status()
        data = response.json()
        result["tests"].append({"name": "Single RPC Request", "status": "passed", "response": data})
        ...
        result["tests"].append({"name": "Batch RPC Request", "status": "passed", "response": batch_data})
        ...
        result["tests"].append({"name": "RPC Introspection", "status": "passed", "response": introspection_data})
    ...
    async def _test_websocket_transport(self, result: Dict[str, Any]):
        ...
        result["tests"].append({"name": "WebSocket Status", "status": "passed", "response": status_data})
        ...
        result["tests"].append({"name": "WebSocket Health", "status": "passed", "response": health_data})
        ...
        result["tests"].append({"name": "WebSocket Metrics", "status": "passed", "response": metrics_data})
    ...
    async def _test_sse_transport(self, result: Dict[str, Any]):
        ...
        response.raise_for_status()
        status_data = response.json()
        result["tests"].append({"name": "SSE Status", "status": "passed", "response": status_data})
        ...
        response.raise_for_status()
        health_data = response.json()
        result["tests"].append({"name": "SSE Health", "status": "passed", "response": health_data})
        ...
        try:
            ...
            result["tests"].append({"name": "SSE Broadcast (No Connections)", "status": "passed", "response": broadcast_data})
            print("  ✅ SSE broadcast (no connections): PASSED")
        except Exception as e:
            result["tests"].append({"name": "SSE Broadcast", "status": "expected_failure", "error": str(e)})
    ...
    async def _test_streamable_http_transport(self, result: Dict[str, Any]):
        ...
        result["tests"].append({"name": "MCP Capabilities", "status": "passed", "response": capabilities_data})
        ...
        response.raise_for_status()
        status_data = response.json()
        result["tests"].append({"name": "MCP Status", "status": "passed", "response": status_data})
        ...
        response.raise_for_status()
        health_data = response.json()
        result["tests"].append({"name": "MCP Health", "status": "passed", "response": health_data})
    ...
    async def _test_stdio_transport(self, result: Dict[str, Any]):
        ...
        response.raise_for_status()
        health_data = response.json()
        result["tests"].append({"name": "STDIO Health", "status": "passed", "response": health_data})
        ...
        result["tests"].append({"name": "STDIO Execute", "status": "passed", "response": execute_data})
...
class SimpleMCPAgent:
    ...
    async def run_comprehensive_test(self, quick_mode=False, verbose=False, list_only=False) -> Dict[str, Any]:
        ...
        try:
            ...
            for tool in tools[:test_limit]:  # Test limited number of tools
                ...
                try:
                    ...
                    if "arguments" in error_msg.lower() or "required" in error_msg.lower():
                        print(f"⚠️  Tool requires arguments, trying with sample data...")
                        try:
            ...
            for resource in resources[:test_limit]:  # Test limited number of resources
                ...
                try:
                    ...
                    error_msg = str(e)
                    test_results["errors"].append(f"Resource {resource_uri}: {error_msg}")
            ...
            for prompt in prompts[:test_limit]:  # Test limited number of prompts
                ...
                try:
                    ...
                    if "arguments" in error_msg.lower():
                        print(f"⚠️  Prompt requires arguments, trying with sample data...")
                        try:
                            sample_args = {"query": "test", "text": "sample text", "input": "test input"}
...
async def main():
    parser = argparse.ArgumentParser(description="Comprehensive MCP Gateway Transport & Server Test Agent")
    parser.add_argument("--transports-only", action="store_true", help="Only test transport protocols, skip MCP server testing")
    ...
    parser.add_argument("--quick", action="store_true", help="Run quick test (1 tool/resource/prompt each)")
    parser.add_argument("--verbose", action="store_true", help="Show full response content (no truncation)")
    parser.add_argument("--list-only", action="store_true", help="Only list capabilities, don't test execution")
    ...
    try:
        ...
        if args.transports_only:
            ...
            for transport_name, result in transport_results.items():
                ...
                print(f"{status_emoji} {transport_name.upper().replace('_', ' ')}: {result['status']}")
        ...
        # Run comprehensive test suite
            ...
            print("=" * 50)
            print(f"🛠️  Tools: {test_results['tools_successful']}/{test_results['tools_tested']} successful")
            print(f"📚 Resources: {test_results['resources_successful']}/{test_results['resources_tested']} successful") 
            print(f"💬 Prompts: {test_results['prompts_successful']}/{test_results['prompts_tested']} successful")
            
            if test_results['errors']:
                print(f"\n⚠️  ERRORS ENCOUNTERED ({len(test_results['errors'])}):")
                for i, error in enumerate(test_results['errors'][:5], 1):  # Show first 5 errors
                    print(f"   {i}. {error}")
                if len(test_results['errors']) > 5:
                    print(f"   ... and {len(test_results['errors']) - 5} more errors")
            
            # Calculate overall success rate
            total_tested = test_results['tools_tested'] + test_results['resources_tested'] + test_results['prompts_tested']
            total_successful = test_results['tools_successful'] + test_results['resources_successful'] + test_results['prompts_successful']
        ...
        if not args.server_id and not args.tool:
            ...
            try:
                transport_results = await gateway.test_all_transports()
                passed_transports = sum(1 for result in transport_results.values() if result["status"] == "passed")
                total_transports = len(transport_results)
                print(f"🚀 Transport verification: {passed_transports}/{total_transports} transports working")
            except Exception as e:
