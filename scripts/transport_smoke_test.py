...
async def test_transport(session: httpx.AsyncClient, token: str, transport_name: str, tests: list) -> dict:
    ...
    for test in tests:
        try:
            ...
            if method == "GET":
                ...
                response = await session.post(url, json=test.get("data", {}), headers=headers)
...
async def main():
    ...
    transport_tests = {
        "JSON-RPC": [
            {"name": "Single RPC Request", "method": "POST", "endpoint": "/rpc", 
             "data": {"jsonrpc": "2.0", "id": "1", "method": "ping", "params": {}}},
            {"name": "Batch RPC Request", "method": "POST", "endpoint": "/rpc/batch",
             "data": [{"jsonrpc": "2.0", "id": "1", "method": "ping", "params": {}}]},
            {"name": "RPC Introspection", "method": "GET", "endpoint": "/rpc/introspection"}
        ],
        "WebSocket": [
        ...
        "STDIO": [
            ...
            {"name": "Execute Command", "method": "POST", "endpoint": "/stdio/execute",
    ...
    async with httpx.AsyncClient(timeout=30.0) as session:
        try:
            ...
            for result in all_results:
                status = "✅" if result["failed"] == 0 else "❌" if result["passed"] == 0 else "⚠️"
