...
/**
 * The Shortcuts component is responsible for rendering a list of shortcuts based on the navigation and shortcuts props.
...
function Shortcuts(props: ShortcutsProps) {
	...
	const [searchResults, setSearchResults] = useState<FlatNavItemType[]>([]);
	const [shortcutItems, setShortcutItems] = useState<FlatNavItemType[]>([]);
				...
				return (
					<Box className={clsx('flex flex-1 items-center', variant === 'vertical' ? 'flex-col' : 'max-h-9')}>
						...
						>
							...
							>
								<SvgIcon sx={{ color: amber[600] }}>lucide:star</SvgIcon>
							</IconButton>
						</Tooltip>
					</Box>
				);
...
function ShortcutMenuItem(props: {
	shortcuts: ShortcutsProps['shortcuts'];
