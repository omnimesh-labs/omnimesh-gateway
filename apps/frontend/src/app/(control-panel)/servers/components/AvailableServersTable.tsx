...
export default function AvailableServersTable({
	...
	const columns = useMemo<MRT_ColumnDef<MCPPackage>[]>(() => [
		{
			...
			Cell: ({ row }) => (
				<Box className="flex items-start space-x-3">
					<SvgIcon size={20} className="mt-1">lucide:package</SvgIcon>
					<Box>
		...
		},
		{
			...
			Cell: ({ row }) => (
				<Box className="flex items-center space-x-1">
					<SvgIcon size={16} className="text-yellow-500">lucide:star</SvgIcon>
					<Typography variant="body2">
						{row.original.github_stars.toLocaleString()}
					</Typography>
				</Box>
			)
		},
		...
		return (
			<Box className="flex items-center justify-center py-12">
				<Box className="text-center">
					<SvgIcon size={48} className="text-gray-300 mb-4">lucide:loader-2</SvgIcon>
	...
	if (servers.length === 0) {
		return (
			<Box className="flex items-center justify-center py-12">
				<Box className="text-center">
					<SvgIcon size={64} className="text-gray-300 mb-4">lucide:search-x</SvgIcon>
