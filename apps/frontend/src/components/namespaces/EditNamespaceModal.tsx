...
import { serverApi, namespaceApi } from '../../lib/api';
...
export function EditNamespaceModal({ namespace, onClose, onUpdate }: EditNamespaceModalProps) {
  ...
  const fetchNamespaceDetails = async () => {
    try {
      const details = await namespaceApi.getNamespace(namespace.id);
      setNamespaceDetails(details);
      // Set selected servers from namespace details
      if (details.servers && Array.isArray(details.servers)) {
        const serverIds = details.servers.map((s: any) => 
          typeof s === 'string' ? s : s.server_id || s.id
        );
        setSelectedServers(serverIds);
      }
    } catch (error) {
      console.error('Failed to fetch namespace details:', error);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await namespaceApi.updateNamespace(namespace.id, {
        name: name.trim(),
      ...
      setError(error instanceof Error ? error.message : 'Failed to update namespace');
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            ...
            <p style={{
              ...
              {selectedServers.length} server(s) selected{namespaceDetails?.servers && ` (currently ${namespaceDetails.servers.length} assigned)`}
            </p>
          </div>
